...
auto getRandomPoint { []() -> Point {
    return {
      static_cast<uint8_t>( random(World::minY, World::maxY) ),
...
volatile Direction lastDirectionPressed {
  Direction::NONE
};
...
void setup() {
  ...
  /*if (ITimer1.attachInterruptInterval(Timing::buttonReadTimeMillis, readButtons)) {
    Serial.println("Timer attached successfully.");
    }*/
  ...
  ///////////////////////////////
 DEBUG_PRINTLN_FLASH("MMA8451 test");


  if (! mma.begin()) {
    DEBUG_PRINTLN_FLASH("Couldnt start");
    while (1);
  }
  DEBUG_PRINTLN_FLASH("MMA8451 found");
...
#pragma message "Inefficient use pointer to head and tail."
  ...
  // set snake direction based off accelerometer tilt direction
...
void doSplashScreen() {
  ...
  while (true) {
    ...
    if (tNow - Timing::lastGameUpdatedTime > Timing::gameUpdateTime_ms) {
      ...
      display.fillRect(15, 20, 100, 32, BLACK); // blank background for text
      display.setTextColor(WHITE);
      ...
      display.drawRect(15, 20, 100, 32, WHITE);         // border box  - 3
      display.setCursor(17, 42);
      display.setTextSize(0);  // font back to normal

      display.println(F("press middle key"));
      display.display();
